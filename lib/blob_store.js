...
// Lê um arquivo JSON do Blob usando get() (em vez de list()+fetch(url) numa
// URL pública) porque o Blob Store deste projeto é PRIVADO — blobs privados
// não têm URL pública acessível por um fetch comum; get() faz a requisição
// já autenticada (com o mesmo OIDC usado para escrever) e devolve o conteúdo
// como stream, que convertemos em texto/JSON aqui.
async function fetchJsonBlob(pathname) {
  const { get } = getBlobLib();
...
// ---------------------------------------------------------------
// Pesquisa de perfil: as 8 perguntas que a RITA faz no próprio chat quando o
// visitante atinge o limite de perguntas da conversa (ver index.html e
// api/survey.js). Guardada separada do registro de acessos porque tem um
// formato diferente (perguntas/respostas, não só nome/cargo/empresa).
// ---------------------------------------------------------------
const SURVEY_LOG_PATH = "survey-log.json";
const SURVEY_LOG_MAX = 2000;

async function appendSurveyResponse(entry) {
  if (!blobConfigured()) return;
  const log = (await fetchJsonBlob(SURVEY_LOG_PATH)) || [];
  log.unshift(entry); // mais recente primeiro
  await putJsonBlob(SURVEY_LOG_PATH, log.slice(0, SURVEY_LOG_MAX));
}

async function getSurveyResponses() {
  if (!blobConfigured()) return [];
  return (await fetchJsonBlob(SURVEY_LOG_PATH)) || [];
}
