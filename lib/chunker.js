...
// Ponto opcional depois do número (ver mesma correção e explicação em
// scripts/chunk_laws.py, 05/ago): artigos 1º a 9º não levam ponto na
// convenção legislativa brasileira, só a partir do Art. 10 em diante.
const ARTIGO_HEADER_RE = /^[ \t]*["'“]?Art\.\s*(\d+[ºo°]?(?:-[A-Z])?)\.?(?=\s)/gm;
const ANEXO_HEADER_RE = /^[ \t]*["'“]?ANEXO\s+([IVXLCDM]+(?:-[A-Z])?)\b/gm;
// Exige que o "§ N" esteja no início de uma linha (mesma âncora já usada em
// INCISO_RE/ALINEA_RE abaixo, e replicada aqui a partir da correção feita em
// scripts/chunk_laws.py em 31/jul). Sem essa âncora, uma referência textual a
// outro parágrafo no meio de uma frase (ex.: "...a informação a que se
// refere o inciso I do § 1º deste artigo...") era confundida com o início de
// um novo parágrafo, cortando o texto no lugar errado.
const PARAGRAFO_RE = /(?:^|\n)[ \t]*(§\s*\d+[ºo°]?(?:-[A-Z])?|Parágrafo único)\.?/g;
const INCISO_RE = /(?:^|\n)[ \t]*([IVXLCDM]{1,6})\s*[-–]\s+/gm;
const ALINEA_RE = /(?:^|\n)[ \t]*([a-z])\)\s+/gm;
