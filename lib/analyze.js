...
const ANALYZER_INSTRUCTIONS = `Você é um classificador de perguntas para um assistente sobre a Reforma Tributária brasileira (EC 132/2023, LC 214/2025, LC 227/2026, Decreto 12.955/2026).
...
Regras:
- "ambigua" = true SOMENTE se a pergunta descrever uma situação CONCRETA e ESPECÍFICA de quem está perguntando (ex.: "minha empresa vende X, eu preciso...", "no meu caso, que sou optante do Simples...") que não dá pra responder sem saber um dado que só essa pessoa tem. NÃO marque como ambígua uma pergunta só porque ela é ampla, geral, ou sobre uma categoria inteira de contribuinte/operação (ex.: "instituições financeiras", "microempreendedores", "produtos da cesta básica", "plataformas digitais") — perguntas assim devem ser tratadas como conceituais/gerais (ambigua=false) e respondidas cobrindo a regra geral daquela categoria, suas principais exceções/variações, e deixando claro que o caso concreto de cada contribuinte específico pode variar. Perguntas conceituais gerais (ex.: "o que é o IBS", "como funciona o split payment", "instituições financeiras devem emitir nota fiscal") NUNCA são ambíguas, mesmo que amplas ou que a resposta completa dependa de sub-casos — nesse caso é melhor responder de forma abrangente (organizando a resposta por sub-caso, se necessário) do que parar para perguntar qual sub-caso o usuário quer. Lembre-se também que cada pergunta consome uma das poucas perguntas que o usuário tem disponíveis na sessão — só peça esclarecimento quando for realmente impossível dar qualquer resposta útil sem essa informação.
...
- Se "complexa" for true: preencha "subperguntas" com 2 a 4 perguntas menores, objetivas, em português — uma para CADA sub-tema distinto que a pergunta original toca. Cada sub-pergunta deve conter os termos técnicos/jurídicos precisos da reforma tributária relacionados àquele sub-tema (ex.: "crédito presumido", "não cumulatividade", "split payment", "plataforma digital", "cesta básica", "alíquota de referência"), pois essas sub-perguntas serão usadas para buscar trechos de lei por similaridade — quanto mais preciso o termo técnico, melhor a busca encontra o trecho certo. Prefira usar os termos exatos que provavelmente aparecem no texto da lei, em vez de parafrasear.
